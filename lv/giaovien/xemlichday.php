...
$ngonngu = mysql_query('select distinct c.MaLop,l.TenLop from lophoc l join ctdaylop c on l.MaLop = c.MaLop where l.MaKH="'.$_POST["khoahoc"].'" and c.MaGV="'.$magv.'"') or die("Không thể tìm ngôn ngữ");
if(mysql_num_rows($ngonngu)>0){
     echo "<h4>Lịch dạy của giảng viên ".$magv." trong khóa ".$_POST["khoahoc"]." </h4>";
     $count = 1;
    while($row = mysql_fetch_array($ngonngu)){
        echo "<hr> Lớp ".$count." - Mã lớp ".$row["MaLop"]." - Tên lớp ".$row["TenLop"].""; $count++;
        $monhoc = mysql_query('select distinct m.mamh, m.tenmh, c.Thu, c1.TenCa, c.MaPhong from ca c1 join ctdaylop c on c1.maca=c.maca join monhoc m on c.MaMH=m.mamh where c.MaLop="'.$row["MaLop"].'" and c.MaGV="'.$magv.'"') or die("Không thể tìm danh sách môn học");
