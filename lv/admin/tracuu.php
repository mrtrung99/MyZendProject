...
<html>
<head>
	<title>Tra cứu văn bằng chứng chỉ</title>
    <meta charset="utf-8">
          ...
          <span class="glyphicon glyphicon-home"></span> Trang chủ
            ...
            <form action="" method="post" enctype="multipart/form-data" class="form-group">
                <div class="row text-center">
                    <fieldset>
                        <legend><strong>Tra cứu bằng cấp theo tên hoặc mã học viên</strong></legend>
                        <input type="text" name="tenhocvien">&nbsp;&nbsp;<br><br><input class="btn btn-primary" type="submit" name="timkiem" value="Tìm kiếm">                         <input class="btn btn-default" type="reset" name="reset1" value="Reset">
                ...
                <div class="row text-center">
                    <fieldset>
                        <legend><strong>Tra cứu bằng cấp hàng loạt</strong></legend>
                    <div class="col-sm-4">
...
if(isset($_POST["timkiem"])){
    $input = $_POST["tenhocvien"];
    $sql = 'select  h.MaHV, h.TenHV, h.NgaysinhHV, h.GioiTinhHV, h.DiaChiHV, h.SDTHV, d.MaKH, d.MaCC, d.MaNN from hocvien h join dscapcc d on h.MaHV=d.MaHV where h.MaHV like "%'.$input.'%" or h.TenHV like "%'.$input.'%"';
    $result = mysql_query($sql) or die("Không thể select dữ liệu");
    if(mysql_num_rows($result)>0){
      //  echo '<h3 class=text-center> Các học viên được cấp chứng chỉ '.$chungchi.' ngôn ngữ '.$ngonngu.' khóa học '.$khoahoc.'</h3> <br>';
        echo ' <table class="table table-bordered  text-center">
        ...
        while($row = mysql_fetch_array($result)){
          //  echo '<option value="'.$row["MaHV"].'"> '.$row["TenHV"].' </option>';
            ...
            echo '<td>';echo $row["MaCC"];
            echo '</td>';

            echo '<td>';echo $row["MaNN"];
            echo '</td>';
            
            echo '<td>';echo '<a href="chitietbangcap.php?kh='.$row["MaKH"].'&nn='.$row["MaNN"].'&cc='.$row["MaCC"].'&mahv='.$row["MaHV"].'" target="_blank"><button class="btn btn-info">Xem chi tiết bằng cấp</button></a>';
            echo '</td>';
            echo '<td>';echo '<button id="diemdanh" value="'.$row["MaHV"] .'" onclick="diemdanh1(this.value, \''.$row["MaKH"].'\',\''.$row["MaNN"].'\',\''.$row["MaCC"].'\' )" class="btn btn-success"> Chi tiết ajax</button>';
            echo '</td>';
